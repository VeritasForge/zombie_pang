...
// 색상 SSOT (N5 fix): computeRageLevel을 호출해 rage 단계로 색상 결정.
...
export class BossHud extends Phaser.GameObjects.Container {
  ...
  private hp: number;
  private readonly chapter: ChapterNumber;

  constructor(scene: Phaser.Scene, x: number, y: number, maxHp: number, chapter: ChapterNumber) {
    super(scene, x, y);
  ...
  private redraw(): void {
    ...
    // 색상 SSOT (N5 fix): rage 단계로 변환 — Ch1~3은 항상 rage 0 (녹), Ch4는 50% 경계, Ch5는 67%/33% 경계
    const rage = computeRageLevel(this.hp, this.maxHp, this.chapter);
    let color: number;
