...
> ⚠️ **ADR-0015(2026-07-20) 갱신**: 본 절의 60초 envelope·보스 클라이맥스·카드 fan-out·챕터 전환 컷씬은 웨이브 클리커 단순화로 제거되었습니다. 현행 코어 루프는 **처치 목표(quota) 클리어 + 도주 한도(escapeLimit) 초과 시 실패**이며 별도 시간 제한이나 보스 조우가 없습니다. 현행은 §1/§2 및 `docs/adr/0015-wave-clicker-simplification.md` 참조. 아래 시퀀스·수치는 이전 설계의 역사적 기록입니다.

### 단위

~~**1 게임 세션 = 1 챕터 = 10 층 = 60초 envelope** (CEO 보스 5초 포함, 상한 68초)~~
**(현행)** 시간 제한 없음. 층은 `floorPlan(F).quota`만큼 처치하면 클리어되고, 도주 누적이 `floorPlan(F).escapeLimit` 도달 시 그 자리에서 run이 종료된다.

### 60초 시퀀스 *(구 설계 기록 — 시간 기반 시퀀스는 제거됨, 현재는 quota 기반)*
...
### 챕터 전환 시퀀스 (Layer 1 → Layer 2) *(구 설계 기록 — 제거됨)*

~~보스 처치 충돌 → freeze frame → 사옥 외관 줌아웃 → 카드 3장 fan-out → 컷씬 → 3택 [다음 챕터]/[카드 자세히]/[정시 퇴근]~~
**(현행)** 층 클리어 시 카드 선택·보스전·컷씬 없이 곧바로 다음 층으로 이어진다. "정시 퇴근" 버튼은 플레이 중 언제든 누를 수 있다.
...
|---|---|---|
| 좀비 처치 / 콤보 / Crit | 결정론 | — |
| Power-up drop rate | 결정론 (평평한 기본율 5%, Coin Tier 없음) | — |
| 층 진행 (quota 클리어 / 도주 실패) | 결정론 | — |

챕터 카드 3장 추첨·사옥 인테리어 재건 순서·골드 폭증은 해당 시스템 자체가 삭제되어 더 이상 존재하지 않는다 — Layer 2 사례는 현재 없음.

### Fail / Success

- **Fail 조건**: 도주 누적이 층별 `escapeLimit(F)` 도달 = run 종료 (밴드 1~2는 5, 밴드 3~4는 4, 밴드 5는 3 — 챕터 고정 5마리가 아니다)
- **Fail 톤 리프레임**: *fail 모달*이 아닌 *조기 퇴근 모달*. 자막 *"오늘은 여기까지 해도 충분합니다."* (ADR-CL-0002 Proposed, 유지)
- **층 클리어 후**: 카드 선택·보스전 없이 곧바로 다음 층. "정시 퇴근" 버튼은 언제든 현재 점수로 종료 가능 (계속 진행과 동등 가중치, default highlight 없음)
- ~~메타 보존: 챕터 중간 정시 퇴근해도 그 챕터 카드·골드·인테리어 보존~~ — 카드/골드/인테리어 자체가 제거되어 해당 없음. 점수·최고 기록은 `end-run.ts`가 항상 저장한다.

### CEO 보스 5초 3-phase *(구 설계 기록 — 보스전 제거로 무효)*
...
카운트다운 압박 텍스트 ("남은 시간 5초!") **금지** 원칙은 유지된다. 다만 "보스 자체 위협"은 더 이상 없음 — 현재는 HUD 숫자(quota/도주 카운트)만으로 긴장을 표현한다.

### 5챕터 누적 난이도 곡선 *(구 설계 기록 — 보스 HP 개념 제거로 무효)*
...
**(현행)** 보스 HP 곱셈 곡선은 존재하지 않는다. 난이도는 `floorPlan(F)`의 quota/cap/spawnRateMs/escapeLimit로만 결정된다(§2 "5개 난이도 밴드" 표 참조).

---

## §4. Meta Progression Bible — 카드 15장 / 출근 도장 / Unlock 3-stage

> ⚠️ **ADR-0015(2026-07-20) 갱신**: 본 절이 기술하는 메타 카드 15장·Unlock 3-stage·출근 도장(Daily Streak)·코인(coin) 재화는 웨이브 클리커 단순화로 전부 삭제되었습니다. 현재 좀비팡에는 메타 성장·코인·스트릭이 없습니다 — 남은 것은 파워업 3종(폭탄/빙결/자석, 처치 시 평평한 기본율 5%로 드롭되어 pickup 탭으로 실제 발동)뿐입니다. 현행은 §1/§2 및 `docs/adr/0015-wave-clicker-simplification.md` 참조. 아래 카드/스트릭 상세는 이전 설계의 역사적 기록입니다.

### 카드 15장 명세표 (제거됨 — 역사적 기록)
...
### Daily Streak — 출근 도장 (제거됨 — 역사적 기록, ADR-0003 Resolved → Option B)
...
### 가변 보상 Layer 2 (제거됨 — 역사적 기록, ADR-0002 Resolved → Option B)

- 사옥 인테리어 재건 = 가변 (방마다 회복 순서/세부 가변)
- 매 처치 1~3% 확률로 골드 폭증 ×5배 = 가변
- 챕터 카드 3장 추첨 = 결정론 균등 추첨

### Power-up 3종 (현행 — 파워업만 유지됨)

| Power-up | 효과 | 지속 | drop rate |
|---|---|---|---|
| 폭탄 | 화면 전체 좀비 즉시 처치 | 즉발 | 평평한 기본율 5%(`BASE_DROP_RATE`) |
| 빙결 | 좀비 스폰·노화 3초 정지 | 3초 | 평평한 기본율 5% |
| 자석 | 100px 반경 좀비 자동 처치 | 3초 | 평평한 기본율 5% |

- 처치 시 3종 중 하나를 균등 추첨해 드롭 여부 결정. 처치 위치에 탭 가능한 pickup이 생성되고, 탭하면 `applyPowerUp`으로 실제 발동(코인/Tier 결합 없음 — 코인 제거로 무의미).
- **동시 활성 한도 2개**(빙결·자석 등 지속형 효과에만 적용, 폭탄은 즉발이라 한도 무관)
- ~~보스 처치 시 30% 확정 drop~~ — 보스전 제거로 더 이상 발생하지 않는다.
- ~~"Tier 3에서 drop 빈도 증가" 표기·정량 표기 금지 원칙~~ — 코인/Tier 자체가 제거되어 해당 없음. 화면 텍스트 0줄 원칙(§1)은 그대로 유지.
...
> ⚠️ **ADR-0015(2026-07-20) 갱신**: 보스 처치(Boss kill) 전용 저지스와 챕터 전환 컷씬은 웨이브 클리커 단순화로 제거되었습니다. Normal/Crit kill, Combo 5+, PowerUp pickup, Wave clear(= 층 클리어), Hit(피격) 저지스는 그대로 유지됩니다. 현행은 §1/§2 및 `docs/adr/0015-wave-clicker-simplification.md` 참조.
...
| ~~Boss kill~~ *(제거됨)* | 8f (133ms) | 12→6px decay | 24p | 2f gold | L1-L4 full | [100,40,100] (240ms) | 600ms |
| Wave clear | — | — | confetti 16p | — | L6 | — | 300ms |
| Hit (피격) | 2f | 4px | — | 1f red | L7 | 100ms | — |

Boss kill 행은 보스전 제거로 더 이상 발생하지 않는다(역사적 기록으로만 유지). `src/adapters/phaser/managers/juice-manager.ts`의 `KillJuiceEvent`는 현재 `"normal" | "crit" | "combo_5+" | "wave_clear"` 4종만 존재한다.
...
| 과장 | 종이 + 명함 |
| 팀장 | 커피잔 + 클립 |
| CEO (탱커 필드 좀비, 보스 아님) | USB 테마 — 일반 처치와 동일한 particle 수(normal 8p / crit 12p), 보스 전용 24p 없음 |

**Adaptive Degradation**: 동시 표시 최대 12p(crit 기준, 구 보스 24p는 보스전 제거로 더 이상 발생하지 않음) → FPS<50 3프레임 연속 시 다운그레이드, 30프레임 alpha decay.
...
- crit kill, 콤보 5+ **2개만**(보스 처치 이벤트는 제거됨; wave clear는 freeze frame + SFX만 있고 haptic은 없음 — `juice-manager.ts` 확인)
