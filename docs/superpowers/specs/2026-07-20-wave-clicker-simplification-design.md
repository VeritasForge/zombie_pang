...
| 파워업 3종 | **유지 + 실제 발동 배선** (폭탄/빙결/자석) — drop은 평평한 기본율, 처치 위치에 pickup 생성 → 탭 시 발동 |
...
파워업은 기존에 도메인/use-case만 있고 화면 배선이 없었다(드롭 계산 후 무시). 이번 범위에서 **실제 발동되도록 배선**한다.

- **드롭**: 좀비 처치 시 평평한 확률(기본율 5%)로 드롭. 처치 위치에 탭 가능한 pickup 아이콘 생성. 일정 시간(약 4.5초) 미획득 시 소멸.
- **발동**: pickup 탭 → `applyPowerUp` use case 호출 → 효과 적용.
  - **폭탄(Bomb)**: 즉발. 화면의 활성 좀비 전부 처치(점수·처치 목표 반영).
  - **빙결(Freeze)**: 3초. 좀비 스폰 정지 + 노화(도주) 정지.
  - **자석(Magnet)**: 3초. 좀비를 화면 중앙으로 끌어당기고 범위 내 진입 시 자동 처치.
- **동시 활성 한도**: 타이머 효과(빙결/자석) 동시 2개까지. 폭탄은 즉발이라 한도 무관. 한도 초과 시 새 pickup 탭 무시(pickup은 유지).
- **use-case 정리**: `apply-powerup.ts`의 메타(카드) 의존을 제거한다. 빙결/자석 지속은 기본 3000ms 고정, 자석 범위는 기본 100px 고정(메타 카드 보너스 없음 — 메타 제거와 정합).
